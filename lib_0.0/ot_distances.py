...
class Fused_Gromov_Wasserstein_distance():
    ...
    def calc_fgw(self,M,C1,C2,t1masses,t2masses):
        transpwgw,log= fgw.fgw_lp((1-self.alpha)*M,C1,C2,t1masses,t2masses,self.loss_fun,G0=None,alpha=self.alpha,verbose=self.verbose,amijo=self.amijo,log=True)      
    ...
    def graph_d(self,graph1,graph2,t1masses,t2masses):
        ...
        for j in range(n2):
            for jj in range(n2):  
                C2[j][jj] = nx.shortest_path_length(graph2.nx_graph,source=j,target=jj)
        ...
        startdist=time.time()
        transpwgw,log=self.calc_fgw(M,C1,C2,t1masses,t2masses) #return the transport matrix and FGWD value
        enddist=time.time()
